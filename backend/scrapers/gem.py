...
# Change city_input argument to not required and default to empty string
parser.add_argument("--city_input", type=str, required=False, default="", help="Enter city name or else skip.")
...
BASEDIR = path.dirname(path.dirname(path.abspath(__file__)))  # Go up one level to backend/
...
def select_state_and_city(state_index=None, city_input=None):
    # If arguments are provided, use them directly (non-interactive mode)
    if state_index is not None:
        ...
        if selected_state in valid_cities and city_input and city_input.strip() != "":
            # Accept city name directly
            if city_input in valid_cities[selected_state]:
            ...
            selected_city = ""
            print("No city provided. Skipping city filter.")
        print(f"\nYou selected: {selected_state}" + (f", {selected_city}" if selected_city else " with no specific city."))
        return selected_state, selected_city

    # Fallback to interactive mode
    print("\nSelect a state:")
    display_options(valid_states)
    state_index = int(input("Enter the state index: ")) - 1
    if state_index < 0 or state_index >= len(valid_states):
        print("Invalid state selection. Exiting.")
        return None, None
    selected_state = valid_states[state_index]
    ...
    if selected_state in valid_cities:
        ...
        city_input = input("Enter city name or leave blank to skip: ").strip()
        if city_input == "":
            selected_city = ""
...
def main():
    try:
        print("gem.py started with arguments:", vars(args))
        state, city = select_state_and_city(args.state_index, args.city_input)
        print(f"Selected state: {state}, city: {city}")
        if state:
            today, tommorrow = select_dates()
            print(f"Selected dates: today={today}, tommorrow={tommorrow}")
            fetch_bid_data(state, city, today, tommorrow)
            print("Data fetch complete.")
        else:
            print("No valid state selected. Exiting.")
    except Exception as e:
        print("Exception in gem.py:", e)
