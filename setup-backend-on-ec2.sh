...
# =============================================================================
# LAVANGAM BACKEND COMPLETE SETUP SCRIPT FOR EC2
...
log "🚀 Starting LAVANGAM Backend Complete Setup on EC2..."
...
# Allow all backend service ports
...
# =============================================================================
log "📝 Creating main application entry point..."

cat > $BACKEND_DIR/main_app.py <<'EOF'
#!/usr/bin/env python3
"""
Main Application Entry Point for LAVANGAM Backend
...
# WebSocket support for E-Procurement
...
cat > $BACKEND_DIR/start_services.sh <<'EOF'
#!/bin/bash
cd "$(dirname "$0")"
source venv/bin/activate
python main_app.py
...
chmod +x $BACKEND_DIR/start_services.sh
...
def main():
    ...
    for service in SERVICES:
        ...
        print(f"{service['name']:<25} (Port {service['port']:<4}) {status}")
...
# =============================================================================
# STEP 11: Create Database Setup Scripts
# =============================================================================
log "🗄️ Creating database setup scripts..."

# Create database initialization script
cat > $BACKEND_DIR/setup_database.py <<'EOF'
#!/usr/bin/env python3
"""
Database Setup Script for LAVANGAM Backend
"""
...
def create_tables():
    ...
    connection = mysql.connector.connect(
        ...
        user=os.getenv('DB_USER', 'lavangam_user'),
        password=os.getenv('DB_PASSWORD', 'Lavangam2024!'),
        database=os.getenv('DB_NAME', 'lavangam_db')
    )
...
chmod +x $BACKEND_DIR/setup_database.py
...
# =============================================================================
# STEP 13: Verification and Status
# =============================================================================
...
# Create a comprehensive management script
cat > $BACKEND_DIR/manage.sh <<'EOF'
#!/bin/bash

# LAVANGAM Backend Management Script
...
case "$1" in
    start)
        ...
        ;;
    stop)
        echo "🛑 Stopping LAVANGAM Backend..."
        sudo systemctl stop lavangam-backend
        sudo systemctl stop nginx
        sudo systemctl stop mysql
        ;;
    restart)
        echo "🔄 Restarting LAVANGAM Backend..."
    ...
    *)
        ...
        echo "  update  - Update backend code"
        exit 1
        ;;
esac
EOF

chmod +x $BACKEND_DIR/manage.sh

# =============================================================================
# STEP 15: Installation Complete
# =============================================================================
log "🎉 LAVANGAM Backend Setup Complete!"
echo ""
echo "=============================================================================="
echo "🚀 LAVANGAM BACKEND SUCCESSFULLY DEPLOYED ON EC2!"
echo "=============================================================================="
echo ""
...
echo "   • Start services: $BACKEND_DIR/manage.sh start"
echo "   • Stop services: $BACKEND_DIR/manage.sh stop"
echo "   • Check status: $BACKEND_DIR/manage.sh status"
echo "   • View logs: $BACKEND_DIR/manage.sh logs"
echo "   • Health check: $BACKEND_DIR/manage.sh check"
echo ""
echo "📋 Next Steps:"
...
LAVANGAM Backend Setup Summary
==============================
...
Management:
- Use: $BACKEND_DIR/manage.sh {start|stop|restart|status|logs|check}
...
log "🎯 Your LAVANGAM backend is now ready on EC2!"
